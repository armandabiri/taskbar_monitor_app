...
- stacks.flutter.logging.no-console-output: print/debugPrint/log > use IntelagLogger
- stacks.flutter.architecture.cloud-abstraction: direct AWS SDK from UI/provider > abstraction layer via use case
- stacks.flutter.ui.use-ui-kit: ElevatedButton/TextField in features > AppPrimaryButton/AppTextField
- stacks.flutter.imports.absolute-package-imports: relative import ../models/order.dart > absolute package import
- stacks.flutter.types.no-unexplained-dynamic: dynamic without comment > explicit type or // dynamic: reason
- stacks.flutter.null-safety.no-unjustified-bang: value! without null guarantee > value ?? fallback or null check
- shared.secrets.no-pii-in-logs: logging PII IntelagLogger.info('token=$token') > anonymized IntelagLogger.info('auth userId=%s', [userId])
- shared.secrets.secure-token-storage: SharedPreferences for tokens > flutter_secure_storage
- shared.secrets.https-only: http:// endpoint > https://
- stacks.flutter.riverpod.no-business-logic-in-provider: business logic in Notifier > extract to UseCase
- stacks.flutter.i18n.no-hardcoded-user-text: Text('Submit') > Text(AppLocalizations.of(context)!.submitButton)
- stacks.flutter.ui.no-setstate-shared-widget: setState in shared widget > Riverpod or const
- stacks.flutter.riverpod.use-notifier: StateNotifier/ChangeNotifier > Notifier/AsyncNotifier
- stacks.flutter.riverpod.one-provider-per-concern: one provider for orders+auth+sync > separate provider per concern
- stacks.flutter.architecture.no-direct-rds: direct RDS connection > API Gateway + Lambda
- shared.secrets.encrypt-local-data: unencrypted local DB > SQLCipher/Isar encryption
- stacks.flutter.testing.generate-tests: feature code without tests > generate tests alongside
- stacks.flutter.architecture.domain-pure-dart: Flutter import in domain layer > pure Dart only
